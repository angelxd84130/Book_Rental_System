...
if (isset($action) && $action=='recover' && isset($id)) {
    ...
    $rs = querydb($sqlcmd, $db_conn);
    if (count($rs) > 0) {
        $sqlcmd = "UPDATE rental SET valid='Y' WHERE id='$id'";
...
//找書
$sqlcmd = "SELECT * FROM book WHERE valid='Y'";
$rs = querydb($sqlcmd, $db_conn);
$arrBook = array();
$arrBookID = array();
if (count($rs) > 0) {
...
$sqlcmd = "SELECT * FROM rental WHERE valid='Y'";
...
foreach ($Contacts AS $item) {
  ...
  $Duedate = '';
  if(isset($BookID)){
	if(!empty($arrBook["$BookID"]))
		$Bookname = $arrBook["$BookID"];
  }
  $ClientID = $item['clientID'];
  $Clientname = '';
  if(isset($ClientID)){
	  if(!empty($arrClient["$ClientID"]))
  ...
  $Valid = $item['valid'];
   $DspMsg = "'確定刪除項目?'";
  $PassArg = "'rental.php?action=delete&id=$id'";
...
?>
<?php
if(!empty($Bookname) && !empty($Clientname)){
?> 
<tr align="center">
  <td>
<?php
...
<?php } else { ?>
  <a href="javascript:confirmation(<?php echo $DspMsg ?>, <?php echo $PassArg ?>)">
  作廢</a>&nbsp;
  <a href="returnrental.php?id=<?php echo $id; ?>">
  還書</a>
  </td>
